...
- `frontend/` — Create React App (react-scripts 5, React 19, JSX sources despite `typescript` devDep). UI text and most code comments are Arabic.
...
- API contract: mounted at `/api/:lang/listings` and `/api/:lang/listings/:id`; `lang` must be `en` or `ar` (anything else → 406). `/api/users/:id` requires a session cookie. Auth routes at `/auth/register|login|logout`. All responses wrap payloads as `{ success, data }`.
...
## Frontend
- `npm start` runs on port 3000 (CRA default). `npm test` is jest watch mode; jest config in `package.json` maps swiper to mocks in `src/__mocks__/`, and `src/setupTests.js` polyfills `ResizeObserver`/`matchMedia`/`scrollTo` for jsdom.
- **Frontend talks to the backend through the CRA proxy.** `frontend/package.json` sets `"proxy": "http://localhost:5000"`, and `src/api/api.js` calls the real routes `/api/:lang/listings` over a relative `/api` base (`DEFAULT_LANG` is `ar` because the UI is fully Arabic; keep exported signatures stable if a language system is added). `src/api/normalize.js` (`normalizeProperty`) unifies the API shape into a defensive frontend shape. Backend defaults to port 5000 (`server.js`) and CORS defaults to `http://localhost:3000` (`app.js`). On fetch failure, components (`List.jsx`, `PropertyListing.jsx`, `PropertyDetails.jsx`) silently fall back to `src/data.json` / `src/data/fallbackProperty.js`, so the UI renders even with no backend running.
